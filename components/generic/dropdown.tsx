...
export default function Dropdown({options,onChange}:Props) {
  ...
  return (
    <div className='col-span-2 text-black w-2/3 h-16 mx-4 hover:cursor-pointer' onClick={()=>{setClicked(!clicked)}}>
        <select onChange={handelChange} className='cursor-pointer w-full h-16 outline-none text-lg font-medium py-3 px-5 rounded-xl shadow-lg pointer-events-auto focus:shadow-xl transition-all '>
