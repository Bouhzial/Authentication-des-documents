...
export default function SideNav({name,image_link,options}:Props) {
  ...
  return (
    <div className='flex flex-col bg-primary-gray justify-between w-1/5 min-w-fit h-screen'>
        ...
        {options && options.map((option:linknav,key:number) => {
              ...
              return (
                <a className={` px-6 hover:shadow-2xl transition-all  
                                duration-300 border-b border-gray-400 h-16 w-full 
                                 flex items-center justify-center ${link.includes(option.link)?'bg-link-text-blue text-white':
                                 'text-link-text-blue hover:bg-blue-900 bg-link-gray hover:text-link-gray'} `} href={option.link}>
                {option.name}
            </a>
              )})}
        </div>   
