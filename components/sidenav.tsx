...
export default function SideNav({name,image_link,options}:Props) {
  return (
    <div className='flex flex-col justify-between w-1/5 h-screen'>
        ...
        {options && options.map((option:linknav) => {
              return (
                <a className='text-link-text-blue hover:scale-105 hover:shadow-xl hover:text-link-gray transition-all hover:bg-blue-900 duration-300 border-b border-gray-400 h-16 w-full bg-link-gray flex items-center justify-center' href={option.link}>
                {option.name}
            </a>
              )})}
        </div>    
